...
# --- ROTTA 2: Il Bot WhatsApp ---
@app.route('/whatsapp', methods=['POST'])
def whatsapp_bot():
    testo_utente = request.values.get('Body', '')
    
    if not testo_utente.strip():
        resp = MessagingResponse()
        resp.message("Invia un messaggio valido da analizzare.")
        return str(resp)
    ...
    system_prompt = """
    Sei un assistente esperto in cybersecurity e prevenzione delle truffe digitali in Italia.
    Analizza il messaggio ricevuto e rispondi in modo chiaro e diretto per WhatsApp con questa struttura:
    RISCHIO: [Basso / Sospetto / Alto]
    ...
    try:
        ...
        risultato_ai = "Errore tecnico durante l'analisi del messaggio."

    resp = MessagingResponse()
    resp.message(risultato_ai)
    return str(resp)
